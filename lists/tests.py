...
class ItemModelTest(TestCase):
    '''testing model of list item'''

    def test_saving_and_retrieving_items(self):
        '''testing saving and retrieving list items functionality'''
        first_item = Item()
...
class HomePageTest(TestCase):
    ...
    def test_redirects_after_POST(self):
        '''test: response with redirect after POST request'''
        response = self.client.post('/', data = {'item_text': 'A new list item'})
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response['location'], '/')

    def test_only_saves_items_with_necessary(self):
        '''test: saving things only when necessary'''
        self.client.get('/')
